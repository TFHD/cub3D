...
void	matricial_transform(t_ray *ray, t_sp_draw *sp_draw, t_sprite *sprite)
{
	...
	sp_draw->sprite_screen.x = (int)((ray->width * 1.0 / 2)
			* (1 + sp_draw->transform.x / sp_draw->transform.y));
	sp_draw->sprite_size.y = abs((int)(ray->height / (sp_draw->transform.y)));
}

void	search_where_draw(t_ray *ray, t_sp_draw *sp_draw)
{
	sp_draw->draw_y.x = -sp_draw->sprite_size.y / 2 + ray->height / 2;
	if (sp_draw->draw_y.x < 0)
		sp_draw->draw_y.x = -ray->pitch;
	sp_draw->draw_y.y = sp_draw->sprite_size.y / 2 + ray->height / 2;
	...
	sp_draw->draw_x.x = -sp_draw->sprite_size.x / 2 + sp_draw->sprite_screen.x;
	if (sp_draw->draw_x.x < 0)
		sp_draw->draw_x.x = 0;
	sp_draw->draw_x.y = sp_draw->sprite_size.x / 2 + sp_draw->sprite_screen.x;
	if (sp_draw->draw_x.y > ray->width)
		sp_draw->draw_x.y = ray->width;
}

void	draw_sprite_line(t_data *data, t_sp_draw *sp_draw,
	t_sprite *sp, t_vec i)
{
	int			dimension;
	mlx_color	color;
...
void	draw_sprite(t_data *data, t_ray *ray, t_sp_draw *sp_draw, t_sprite *sp)
{
	int			x;
	int			y;
	int			type;
