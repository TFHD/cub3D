...
/*   Updated: 2025/04/01 16:39:16 by sabartho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_colors(char *line, int rgb[3], int *colors_collect)
{
	...
	if (error)
		return (print_error(ERROR_COLORS, line, TRUE));
	(*colors_collect)++;
	return (0);
}
...
void	search_content(char *line, int *error, int *content[2], t_data *data)
{
	...
	if (!ft_strncmp(line + i, "NO ", 3))
		...
		*error = print_error(ERROR_UNKNOW_TYPE, line, TRUE);
	free(line);
}
						...
						int colors_collect, int fd)
{
	...
	if (line == NULL)
		line = ft_strdup("\n");
	while (line && !error && textures_collect + colors_collect < 6)
	{
		data->line_read++;
		search_content(line, &error, content, data);
		if (error)
			break ;
		if (textures_collect + colors_collect != 6)
			line = get_next_line(fd);
	}
	if (!line && textures_collect + colors_collect != 6)
		error = 2;
